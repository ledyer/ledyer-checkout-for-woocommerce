 ...
 * @return void
 */
// /class-ledyer-confirmation.php(55): wc_ledyer_confirm_ledyer_order('71', true)
function wc_ledyer_confirm_ledyer_order($order_id, $ledyer_pending)
{
	...
	if (isset($request['orderAmount']) || isset($request['id'])) {
		...
		if (!$ledyer_pending) {
			...
			// TODO: set status "on hold" + populate order info (now we get "Failed to retrieve the order from Ledyer")
