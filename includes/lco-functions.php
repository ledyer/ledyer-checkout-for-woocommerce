...
function lco_create_or_update_order()
{
	...
	if (
		WC()->session->get("lco_wc_order_id") &&
		...
		if (
			!$ledyer_order ||
			(is_object($ledyer_order) && is_wp_error($ledyer_order)) ||
			$ledyer_order["orderId"] !== $ledyer_order_id ||
			$ledyer_order["sessionId"] !== $ledyer_session_id
		) {
			...
			WC()->session->set("lco_wc_settings", [
...
{
	if (isset($_GET["confirm"]) && "yes" === $_GET["confirm"] && isset($_GET["lco_wc_session_id"])) {
		// phpcs:ignore WordPress.Security.NonceVerification -- No nonce possible on this page.
		return true;
	}
...
function lco_wc_show_another_gateway_button()
{
	...
	if (count($available_gateways) > 1) {
		...
		$select_another_method_text =
			isset($settings["select_another_method_text"]) && "" !== $settings["select_another_method_text"]
				? $settings["select_another_method_text"]
				: __("Select another payment method", "ledyer-checkout-for-woocommerce");
		?>
...
function lco_wc_add_extra_checkout_fields()
{
	...
	<?php do_action("lco_wc_after_extra_fields");
}
...
function wc_ledyer_confirm_ledyer_order($order_id)
{
	...
	if (!empty($order->get_date_paid()) || $order->has_status(["on-hold"])) {
		return;
	}
	...
	switch ($ledyer_payment_status["status"]) {
		case LedyerPaymentStatus::orderPending:
			if (!$order->has_status(["on-hold", "processing", "completed"])) {
				$note = sprintf(
					__('New session created in Ledyer with Payment ID %1$s. %2$s', "ledyer-checkout-for-woocommerce"),
					$payment_id,
		...
		case LedyerPaymentStatus::paymentPending:
			if (!$order->has_status(["on-hold", "processing", "completed"])) {
				$note = sprintf(
					__('New payment created in Ledyer with Payment ID %1$s. %2$s', "ledyer-checkout-for-woocommerce"),
					$payment_id,
					$ledyer_payment_status["note"]
				);
				$order->update_status("on-hold", $note);
	...
	if ($ackOrder) {
		...
		$ledyer_update_order = ledyer()->api->update_order_reference($payment_id, [
