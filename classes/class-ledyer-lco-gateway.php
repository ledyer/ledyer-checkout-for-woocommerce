...
if (class_exists("WC_Payment_Gateway")) {
	...
	class LCO_Gateway extends \WC_Payment_Gateway
	{
		public function __construct()
		{
			...
			$this->method_description = __(
				"The current Ledyer Checkout replaces standard WooCommerce checkout page.",
				"ledyer-checkout-for-woocommerce"
			);
			$this->has_fields = false;
			$this->supports = apply_filters("lco_wc_supports", [
				"products",
				"subscriptions",
			...
			\add_filter("script_loader_tag", [$this, "add_data_attributes"], 10, 2);

			\add_action("woocommerce_checkout_order_processed", [$this, "wc_order_created"], 10, 3);
			\add_action("woocommerce_thankyou_" . $this->id, [$this, "show_thank_you_snippet"]);
			\add_action("woocommerce_thankyou", "lco_unset_sessions", 100, 1);

			\add_action("woocommerce_admin_order_data_after_billing_address", [$this, "ledyer_order_billing_fields"], 10, 1);
			\add_action(
				"woocommerce_admin_order_data_after_shipping_address",
				[$this, "ledyer_order_shipping_fields"],
				10,
				1
			);

			// Validate shipping and billing custom fields
			\add_action("woocommerce_process_shop_order_meta", [$this, "lom_validate_lom_edit_ledyer_order"], 45, 1);
			...
			\add_action("woocommerce_process_shop_order_meta", [$this, "ledyer_order_save_custom_fields"], 50, 1);
		}
		...
		public function enqueue_scripts()
		{
			...
			wp_register_script("lco-iframe", $scriptSrcUrl, ["jquery", "wc-cart"], LCO_WC_VERSION, true);
			...
			if (
				null !== WC()->customer &&
				method_exists(WC()->customer, "get_billing_email") &&
				!empty(WC()->customer->get_billing_email())
			) {
			...
			$standard_woo_checkout_fields = apply_filters("lco_ignored_checkout_fields", [
				"billing_first_name",
				"billing_last_name",
				"billing_address_1",
				"billing_address_2",
			...
			$checkout_localize_params = [
				...
				"must_login_message" => apply_filters(
				...
				"no_shipping_message" => apply_filters(
		...
		public function process_payment_handler($order_id)
		{
			...
			if ($order && $ledyer_order) {
				$customer_billing = isset($ledyer_order["customer"]["billingAddress"])
					? $ledyer_order["customer"]["billingAddress"]
					: false;
				$customer_shipping = isset($ledyer_order["customer"]["shippingAddress"])
					? $ledyer_order["customer"]["shippingAddress"]
					: false;

				$company_name = !empty($customer_billing["companyName"])
					? $customer_billing["companyName"]
					: (!empty($customer_shipping["companyName"])
						? $customer_shipping["companyName"]
						: "");
				...
				if (isset($ledyer_order["customer"]["shippingAddress"])) {
					$order->update_meta_data(
						"_shipping_attention_name",
						sanitize_text_field($ledyer_order["customer"]["shippingAddress"]["attentionName"])
					);
					$order->update_meta_data(
						"_shipping_care_of",
						sanitize_text_field($ledyer_order["customer"]["shippingAddress"]["careOf"])
					);
				}
				...
				if (isset($ledyer_order["customer"]["shippingAddress"]["contact"])) {
					$order->update_meta_data(
						...
						sanitize_text_field($ledyer_order["customer"]["shippingAddress"]["contact"]["firstName"])
					);
					$order->update_meta_data(
						"_shipping_last_name",
						sanitize_text_field($ledyer_order["customer"]["shippingAddress"]["contact"]["lastName"])
					);
					$order->update_meta_data(
						"_shipping_phone",
						sanitize_text_field($ledyer_order["customer"]["shippingAddress"]["contact"]["phone"])
					);
					$order->update_meta_data(
						"_shipping_email",
						sanitize_text_field($ledyer_order["customer"]["shippingAddress"]["contact"]["email"])
					);
				}
				...
				if (isset($ledyer_order["customer"]["billingAddress"])) {
					$order->update_meta_data(
						"_billing_attention_name",
						sanitize_text_field($ledyer_order["customer"]["billingAddress"]["attentionName"])
					);
					$order->update_meta_data(
						"_billing_care_of",
						sanitize_text_field($ledyer_order["customer"]["billingAddress"]["careOf"])
					);
				}
			...
			if ($handle == "lco-iframe") {
				...
				return str_replace(
					...
					'<script data-env="' .
						...
						'"  data-session-id="' .
						$lco_order_id .
						'" data-container-id="lco-iframe" data-buy-button-color="' .
						$buy_button_color .
						'" data-no-padding="' .
						$no_padding .
						'" ',
   ...
   ?>
		...
		{
			if (in_array($payment_method, ["ledyer_payments", "lco"])) {
				return true;
			}
			return false;
		}
