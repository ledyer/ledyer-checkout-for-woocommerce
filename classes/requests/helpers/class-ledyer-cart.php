...
class Cart
{
	...
	public function process_coupons()
	{
			...
			foreach (WC()->cart->get_coupons() as $coupon_key => $coupon) {
				...
				if ("smart_coupon" === $coupon->get_discount_type()) {
					...
					if (wc_tax_enabled() && "yes" === $apply_before_tax) {
						$coupon_amount = 0;
						$coupon_description =
							__("Gift card", "ledyer-checkout-for-woocommerce") .
							" (amount: " .
		...
		if (class_exists("WC_GC_Gift_Cards")) {
			...
			$giftcards_used = WC_GC()->giftcards->cover_balance(
				$totals_before_giftcard,
				WC_GC()->giftcards->get_applied_giftcards_from_session()
			);
		...
		if (!empty(WC()->cart->applied_gift_cards)) {
			foreach (WC()->cart->applied_gift_cards as $coupon_key => $code) {
				...
				$coupon_amount = isset(WC()->cart->applied_gift_cards_amounts[$code])
					? -WC()->cart->applied_gift_cards_amounts[$code] * 100
					: 0;
				$coupon_tax_amount = "";
				$label = apply_filters(
					"yith_ywgc_cart_totals_gift_card_label",
					esc_html(__("Gift card:", "yith-woocommerce-gift-cards") . " " . $code),
					$code
				);
				$giftcard_sku = apply_filters(
					"lco_yith_gift_card_sku",
					esc_html(__("giftcard", "ledyer-checkout-for-woocommerce")),
					$code
				);
		...
		if (!empty(WC()->session->get("pw-gift-card-data"))) {
			...
			foreach ($pw_gift_cards["gift_cards"] as $code => $value) {
				...
				$gift_card_sku = apply_filters(
					"lco_pw_gift_card_sku",
					esc_html__("giftcard", "ledyer-checkout-for-woocommerce"),
					$code
				);
				$gift_card = [
					"type" => "giftCard",
