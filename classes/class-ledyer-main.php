...
class Ledyer_Checkout_For_WooCommerce
{
	...
	public function actions()
	{
		...
		add_action("rest_api_init", function () {
			register_rest_route("ledyer/v1", "/notifications/", [
				"methods" => "POST",
		...
		add_action("woocommerce_checkout_fields", [$this, "modify_checkout_fields"], 20, 1);

		add_action("schedule_process_notification", [$this, "process_notification"], 10, 1);
	}
	...
	public function handle_notification($request)
	{
		...
		$scheduleId = as_schedule_single_action(time() + 120, "schedule_process_notification", [$ledyer_order_id]);
	...
	public function process_notification($ledyer_order_id)
	{
		...
		if ($ackOrder) {
			...
			$ledyer_update_order = ledyer()->api->update_order_reference($ledyer_order_id, [
	...
	public function on_plugins_loaded()
	{
		...
		add_filter("plugin_action_links_" . plugin_basename(LCO_WC_MAIN_FILE), [$this, "plugin_action_links"]);
	}
	...
	public function plugin_action_links($links)
	{
		...
		$plugin_links = ['<a href="' . $setting_link . '">' . __("Settings", "ledyer-checkout-for-woocommerce") . "</a>"];
	...
	public function modify_checkout_fields($checkout_fields)
	{
		...
		if ("lco" === WC()->session->get("chosen_payment_method")) {
			...
			$checkout_fields["billing"]["lco_shipping_data"] = [
				"type" => "hidden",
				"class" => ["lco_shipping_data"],
				"default" => "",
			];
		}
