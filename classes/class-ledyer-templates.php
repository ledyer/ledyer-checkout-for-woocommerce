...
class Templates
{
	...
	public function filters()
	{
		...
		\add_filter("wc_get_template", [$this, "override_template"], 999, 2);
		// Unrequire WooCommerce Billing State field.
		\add_filter("woocommerce_billing_fields", [$this, "unrequire_wc_billing_state_field"]);
		// Unrequire WooCommerce Shipping State field.
		\add_filter("woocommerce_shipping_fields", [$this, "unrequire_wc_shipping_state_field"]);
		// Chage admin shipping fields in edit order admin panel
		\add_filter("woocommerce_admin_shipping_fields", [$this, "change_admin_shipping_fields"], 10, 1);
	}

	public function actions()
	{
		\add_action("wp_footer", [$this, "check_that_lco_template_has_loaded"]);
		...
		add_action("lco_wc_before_snippet", [$this, "add_wc_form"], 10);
	}
		...
		if (is_checkout()) {
			...
			if ("checkout/form-checkout.php" === $template_name) {
				...
				if (locate_template("woocommerce/ledyer-checkout.php")) {
					...
					$ledyer_checkout_template = apply_filters(
						"lco_locate_checkout_template",
						LCO_WC_PLUGIN_PATH . "/templates/ledyer-checkout.php",
						$template_name
					);
				}
				...
				if (array_key_exists("lco", $available_gateways)) {
					...
					if (
						null === WC()->session->get("chosen_payment_method") ||
	...
	public function check_that_lco_template_has_loaded()
	{
		if (
			is_checkout() &&
			array_key_exists("lco", WC()->payment_gateways->get_available_payment_gateways()) &&
			"lco" === lco_wc_get_selected_payment_method() &&
			...
			$url = add_query_arg(
				[
					"lco-order" => "error",
					"reason" => base64_encode(
						__("Failed to load Ledyer Checkout template file.", "ledyer-checkout-for-woocommerce")
					),
